...
real2DFFTData::real2DFFTData(int r, int c, fftw_real *pMem /* = 0 */)
{
	x = r; y = c;
	mem = NULL;
	out = NULL;
	...
	_RPT1(_CRT_WARN, "power size = %d\n", x*(c/2+1));

	if (!pMem)
	{
		if (!(mem = new fftw_real[x*y+x*(c/2+1)+2*x*(c/2+1)+x+(c/2+1)]))
		{
			_RPT0(_CRT_WARN,"real2DFFTData: malloc 2 fail\n");
			memDeInit();
		} 
		else 
		{
	...
	{
		_RPT0(_CRT_WARN, "real2DFFTData: malloc 3 fail\n");
		memDeInit();
	}
	if (!(realXSum = new fftw_real[x]))
	{
		_RPT0(_CRT_WARN, "real2DFFTData: malloc 3a fail\n");
		memDeInit();
	}
	if (!(imagXSum = new fftw_real[x]))
	{
		_RPT0(_CRT_WARN, "real2DFFTData: malloc 3b fail\n");
		memDeInit();
	}
...
void real2DFFTData::memDeInit(void)
{
	...
	if (imagXSum) 
		delete [] imagXSum;
	imagXSum = NULL;
	_RPT0(_CRT_WARN, "real2DFFTData: DeInit Out\n");
}
...
{  
	maxPower = totalPower = 0.0; minPower = DBL_MAX;
	double temp;
...
void real2DFFTData::findYSum(int start, int stop)
{  
...
void real2DFFTData::powerSpecAverage()
{
	...
	memset(ySum, 0, (y/2 + 1) * sizeof(fftw_real));
	xSumMin = ySumMin = DBL_MAX;
	xSumMax = ySumMax = -DBL_MAX;
...
{
	data = d;
	_RPT2(_CRT_WARN, "realFFT init: X %d Y %d\n", data->getXSize(), data->getYSize());
...
real2DFFT::~real2DFFT()
{
	_RPT0(_CRT_WARN, "realFFT DeInit\n");
	fftw_destroy_plan(fwdPlan);
	fftw_destroy_plan(invPlan);
	_RPT0(_CRT_WARN, "realFFT DeInit done\n");
}
